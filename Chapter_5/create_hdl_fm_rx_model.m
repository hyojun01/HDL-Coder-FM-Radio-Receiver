% create_hdl_fm_rx_model.m
% HDL-friendly noncoherent FM receiver example
%
% Top-level:
%   MessageTone -> FM_Mod -> ToFixedPoint -> FM_RX_Core -> Scope
%
% HDL target:
...
add_block('simulink/Sources/Sine Wave', [model '/MessageTone'], ...
    'Position', [x0 y0 x0+70 y0+30], ...
    'Amplitude', '0.8', ...
    'Frequency', '0.01', ...
    'SampleTime', '1');

add_block('commdigbbndam3/FM Modulator Baseband', [model '/FM_Mod'], ...
    'Position', [x0+dx y0-10 x0+dx+100 y0+40], ...
    'FrequencyDeviation', '0.05', ...
    'SamplePerSymbol', '1');

add_block('simulink/Signal Attributes/Data Type Conversion', [model '/ToFixedPoint'], ...
    'Position', [x0+2*dx y0 x0+2*dx+100 y0+30], ...
    'OutDataTypeStr', 'fixdt(1,16,14)');

add_block('simulink/Ports & Subsystems/Subsystem', [model '/FM_RX_Core'], ...
    'Position', [x0+3*dx y0-30 x0+3*dx+180 y0+70]);

add_block('simulink/Sinks/Scope', [model '/Scope'], ...
    'Position', [x0+5*dx y0 x0+5*dx+80 y0+30]);

add_line(model, 'MessageTone/1', 'FM_Mod/1', 'autorouting', 'on');
add_line(model, 'FM_Mod/1', 'ToFixedPoint/1', 'autorouting', 'on');
...
set_param([model '/Note'], ...
    'Position', [35 20 520 80], ...
    'Text', sprintf(['HDL-friendly FM noncoherent receiver core\n' ...
                     'In1 -> Differentiator -> Abs -> 4-tap LPF -> DC blocker -> Out1\n' ...
                     'HDL target: fm_rx_model/FM_RX_Core']));
